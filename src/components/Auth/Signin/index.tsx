...
const Signin = () => {
  ...
  // Use Effect to redirect if authenticated
  useEffect(() => {
    if (isAuthenticated && user) {
      toast.success('Welcome back, ' + user.name)
      router.push('/') // Or previous page
    }
            ...
            <div>
              <form onSubmit={handleSubmit(onSubmit)}>
                ...
                <button
                  type='submit'
                  disabled={loading}
                  className='w-full flex justify-center font-medium text-white bg-dark py-3 px-6 rounded-lg ease-out duration-200 hover:bg-blue mt-7.5 disabled:bg-gray-4 disabled:cursor-not-allowed'
                >
                  {loading ? 'Signing in...' : 'Sign in to account'}
                </button>

                <div className='mt-4 text-center'>
                  <p>Default Account: admin@gmail.com / 123456</p>
                </div>
